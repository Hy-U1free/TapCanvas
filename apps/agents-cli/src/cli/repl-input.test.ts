...
function readComposerBodyRows(lines: string[]): string[] {
  const plainLines = lines.map((line) => line.replace(ANSI_ESCAPE, ""));
  const bottomBorderIndex = plainLines.findIndex(
    (line, index) => index > 1 && line.startsWith("└"),
  );
  assert.ok(bottomBorderIndex > 1, "composer panel bottom border should be present");
  return plainLines.slice(2, bottomBorderIndex).map((line) => {
    const withoutLeftBorder = line.startsWith("│ ") ? line.slice(2) : line;
    const rightBorderIndex = withoutLeftBorder.lastIndexOf(" │");
    const body =
      rightBorderIndex >= 0
        ? withoutLeftBorder.slice(0, rightBorderIndex)
        : withoutLeftBorder;
    return body.trimEnd();
  });
}

function reconstructComposerInput(lines: string[]): string {
  return readComposerBodyRows(lines)
    .map((row, index) => {
      if (index === 0) return row.replace(/^You: /u, "");
      if (row.startsWith("… ")) return `\n${row.slice(2)}`;
      return row.startsWith("  ") ? row.slice(2) : row;
    })
    .join("");
}
...
test("composer panel soft-wraps long input instead of truncating it", () => {
  ...
  assert.equal(reconstructComposerInput(panel.lines), internal.composerInput);
  assert.ok(readComposerBodyRows(panel.lines).length > 1);
...
test("composer panel preserves explicit multiline input while wrapping later rows", () => {
  ...
  assert.equal(reconstructComposerInput(panel.lines), internal.composerInput);
  assert.ok(readComposerBodyRows(panel.lines).some((line) => line.startsWith("… ")));
  assert.ok(readComposerBodyRows(panel.lines).length > 2);
});
