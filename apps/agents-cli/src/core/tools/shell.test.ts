...
function readFileCommand(filePath: string): string {
  ...
  return `${command} ${quoteShellArgument(filePath)}`;
}

function changeDirectoryAndReadFileCommand(root: string, relativePath: string): string {
  if (process.platform === "win32") {
    return `cd /d ${quoteShellArgument(root)} && type ${relativePath.replace(/\//g, "\\\\")}`;
  }
  return `cd ${quoteShellArgument(root)} && cat ${relativePath}`;
}

function listTreeCommand(target: string): string {
  if (process.platform === "win32") {
    return `dir /s /b ${target}`;
  }
  return `find ${target} -maxdepth 3 -type f`;
}
...
function printLiteralCommand(value: string): string {
  ...
  return `printf '%s' ${quoteShellArgument(value)}`;
}

function createFileCommand(filePath: string): string {
  if (process.platform === "win32") {
    return `type nul > ${quoteShellArgument(filePath)}`;
  }
  return `touch ${quoteShellArgument(filePath)}`;
}
...
test("bash restricts project-data access to declared localResourcePaths", async () => {
  ...
  const allowed = await shellTool.execute(
    {
      command: changeDirectoryAndReadFileCommand(
        root,
        "project-data/users/u1/projects/p1/note.txt",
      ),
    },
    {
      cwd,
  ...
  const denied = await shellTool.execute(
    {
      command: listTreeCommand("project-data"),
    },
    {
      cwd,
      depth: 0,
...
test("bash does not block plain quoted project-data text when no filesystem command runs", async () => {
  ...
  const allowed = await shellTool.execute(
    {
      command: printLiteralCommand("project-data/users/u1/projects/p1"),
    },
    {
      cwd,
      depth: 0,
...
test("bash blocks writes when capability writableRoots are empty", async () => {
  ...
  const denied = await shellTool.execute(
    {
      command: createFileCommand("blocked.txt"),
    },
    {
      cwd,
      depth: 0,
...
test("bash blocks read paths outside capability readableRoots", async () => {
  ...
  const denied = await shellTool.execute(
    {
      command: readFileCommand(path.join(deniedDir, "no.txt")),
    },
    {
      cwd,
      depth: 0,
