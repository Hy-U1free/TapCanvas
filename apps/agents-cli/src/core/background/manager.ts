...
import { resolvePlatformShellInvocation } from "../platform-shell.js";
...
export class BackgroundTaskManager {
  ...
  start(input: { command: string; cwd: string; requestedBy: string; env?: Record<string, string> }): BackgroundTaskRecord {
    ...
    const shell = resolvePlatformShellInvocation(command);
    ...
    let processResult:
      | { status: "completed" | "failed"; error?: string; exitCode?: number }
      | undefined;
    let outputFinished = false;
    let finalized = false;
    const finalize = () => {
      if (finalized || !processResult || !outputFinished) return;
      finalized = true;
      this.finishTask(record.id, processResult);
    };
    ...
    stream.on("error", (error) => {
      processResult = {
        status: "failed",
        error: `Unable to write background task output: ${error.message}`,
      };
      outputFinished = true;
      finalize();
    });
