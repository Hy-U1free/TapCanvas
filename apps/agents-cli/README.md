...
`bash`、`exec_command` 与 `background_run` 都使用宿主平台 shell：Windows 使用 `ComSpec`（通常为 `cmd.exe`），其他平台使用 `/bin/sh`。因此命令语法应与当前运行平台匹配。
...
- `src/core/turn-engine.ts` / `src/core/message-compaction.ts`：模型调用前按字符预算压缩过长历史；保留区间从完整 assistant 回合边界开始，并在保留区间含 tool output 时向前带回对应的 assistant tool call，避免产生孤立工具结果。
