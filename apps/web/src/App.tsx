...
import { AppShell, Group, Box, Badge, useMantineColorScheme } from '@mantine/core'
...
import StudioHeader from './ui/studio/StudioHeader'
...
function CanvasApp({ routeKey }: { routeKey?: string }): JSX.Element {
  ...
  const compactHeader = useMediaQuery('(max-width: 93.75em)', false, { getInitialValueInEffect: false })
  ...
  const [headerPointsLoading, setHeaderPointsLoading] = React.useState(Boolean(auth.user && !auth.user.guest))
  ...
  return (
    ...
    >
      <AppShell.Header className="app-shell-header" role="presentation" aria-hidden="true" />
      ...
      <BodyPortal>
        <div className="tc-studio tc-studio__portals">
        <div className="app-header-overlay">
          <StudioHeader
            compact={Boolean(compactHeader)}
            ownerLabel={formatStudioOwnerLabel(currentOwnerType)}
            ownerTone={currentOwnerType === 'shot' ? 'orange' : currentOwnerType === 'chapter' ? 'blue' : 'gray'}
            hostDescription={studioHostDescription}
            isDirty={isDirty}
            saving={saving}
            projectName={currentProject?.name || ''}
            onProjectNameChange={(name) => setCurrentProject({ ...(currentProject || {}), name })}
            onProjectNameBlur={async () => {
              try {
                if (currentProject?.id && currentProject.name) {
                  await upsertProject({ id: currentProject.id, name: currentProject.name })
                }
              } catch (error: unknown) {
                toast(`项目名称保存失败：${resolveErrorMessage(error, '未知错误')}`, 'error')
              }
            }}
            showAccountActions={Boolean(auth.user && !auth.user.guest)}
            showAiWorkbench={Boolean(isAdmin || isProjectOwner)}
            points={headerTeam ? Math.max(0, Number(headerTeam.creditsAvailable || 0)) : null}
            pointsLoading={headerPointsLoading}
            rechargeLoading={headerRechargeLoading}
            onOpenAiWorkbench={() => setHeaderAdminWorkbenchOpen(true)}
            onRecharge={() => setHeaderRechargeOpen(true)}
            onSave={doSave}
            ...
            onToggleLanguage={() => {
              const nextLanguage = currentLang === 'zh' ? 'en' : 'zh'
              setLanguage(nextLanguage)
              setCurrentLang(nextLanguage)
            }}
            onHelp={() => setFeatureTourOpen(true)}
            tapshowUrl="/tapshow"
