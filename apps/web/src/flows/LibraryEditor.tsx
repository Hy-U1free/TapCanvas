...
export default function LibraryEditor({ flowId, onClose }: Props) {
  ...
  return (
    <div className="tc-library-editor" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', zIndex: UI_LAYERS.fullscreenWorkspace, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="tc-library-editor__panel" ref={rootRef} style={{ background: 'var(--mantine-color-default)', color: 'inherit', borderRadius: 12, overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,.35)', border: '1px solid rgba(127,127,127,.25)' }}>
        <div className="tc-library-editor__main" style={{ display: 'flex', flexDirection: 'column', minWidth: 0, minHeight: 0 }}>
          <div className="tc-library-editor__header" style={{ padding: 10, borderBottom: '1px solid rgba(127,127,127,.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
            <Title className="tc-library-editor__title" order={5}>工作流编辑</Title>
            <Group className="tc-library-editor__actions" gap="xs" wrap="wrap">
              <Select className="tc-library-editor__select" size="xs" placeholder="选择服务端工作流" data={serverList.map(f=>({ value: f.id, label: f.name }))} value={currentId} onChange={(v)=> v && loadById(v)} searchable clearable />
          ...
          <div className="tc-library-editor__canvas" style={{ flex: 1, minHeight: 0 }}>
            <ReactFlowProvider>
              <ReactFlow
