...
import { ActionIcon, Badge, Button, Group, Image, Portal, SegmentedControl, Stack, Text, TextInput, Textarea, Title } from '@mantine/core'
import {
  IconBrain,
  IconLayoutGrid,
  IconSearch,
...
import { resolveViewportFloatingPosition } from '../utils/floatingPosition'
...
export default function NanoComicStoryboardTab({
  ...
  const updateMentionMenuPosition = React.useCallback((textValue: string, caretIndex: number) => {
    ...
    const width = Math.min(320, Math.max(1, window.innerWidth - 16))
    const maxHeight = Math.min(220, Math.max(1, window.innerHeight - 16))
    const position = resolveViewportFloatingPosition({
      anchorLeft: textareaRect.left + left + 12,
      anchorTop: textareaRect.top + top,
      anchorBottom: textareaRect.top + top + lineHeight,
      floatingWidth: width,
  ...
  React.useEffect(() => {
    ...
    const handleViewportChange = () => {
      ...
      if (!meta) return
      updateMentionMenuPosition(selectedShotPromptText, meta.at)
    }
  ...
  }, [mentionOpen, selectedShotPromptText, updateMentionMenuPosition])
              ...
              <Stack className="nano-comic-storyboard__editor-stack" gap="md">
                ...
                <div className="nano-comic-storyboard__editor-scroll">
                  <div className="nano-comic-storyboard__prompt-panel">
                    ...
                    {mentionOpen && mentionMenuPosition ? (
                      <Portal>
                        <div
