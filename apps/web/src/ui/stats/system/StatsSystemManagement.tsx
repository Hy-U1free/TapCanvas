...
export default function StatsSystemManagement({ className }: { className?: string }): JSX.Element {
  ...
  const mobileNavigation = useMediaQuery('(max-width: 48em)', false, { getInitialValueInEffect: false }) === true
        ...
        <PanelCard className="stats-system-sidebar glass" padding="compact" miw={220} maw={260}>
          <Stack className="stats-system-sidebar-stack" gap="xs">
            ...
            {STATS_SYSTEM_NAV_GROUPS.map((group) => {
              ...
              return (
                <Menu
                  className="stats-system-sidebar-menu"
                  key={group.key}
                  trigger={mobileNavigation ? 'click' : 'hover'}
                  openDelay={80}
                  closeDelay={120}
                  position={mobileNavigation ? 'bottom-start' : 'right-start'}
                  withinPortal
                >
                  <Menu.Target>
                    <UnstyledButton
