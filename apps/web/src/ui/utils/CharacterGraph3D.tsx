...
import { resolveViewportFloatingPosition } from './floatingPosition'
...
export function CharacterGraph3D(props: {
  ...
  return (
    <div className="character-graph-3d-root" ref={rootRef} style={{ position: 'relative', width: '100%', height: '70vh', minHeight: 420 }}>
      ...
      {!!hover && (() => {
        const containerWidth = Math.max(1, rootRef.current?.clientWidth || 1)
        const containerHeight = Math.max(1, rootRef.current?.clientHeight || 1)
        const width = Math.min(260, Math.max(1, containerWidth - 16))
        const height = Math.min(112, Math.max(1, containerHeight - 16))
        const position = resolveViewportFloatingPosition({
          anchorLeft: hover.x + 14,
          anchorTop: hover.y,
          anchorBottom: hover.y,
          floatingWidth: width,
