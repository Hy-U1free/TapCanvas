...
import { ActionIcon, Badge, Button, FocusTrap, Group, Menu, Modal, Paper, ScrollArea, Stack, Text, Textarea, Tooltip } from '@mantine/core'
...
export type AiChatMode = 'compact' | 'expanded' | 'maximized'

type AiChatPreferenceMode = Exclude<AiChatMode, 'maximized'>

export function resolveAiChatPresentationMode(mode: AiChatMode, narrowViewport: boolean): AiChatMode {
  if (mode === 'maximized') return 'maximized'
  if (narrowViewport && mode === 'expanded') return 'compact'
  return mode
}

export function resolveAiChatVisualStatus(
  sending: boolean,
  latestMessageKind: 'progress' | 'result' | 'error' | null | undefined,
): '就绪' | '运行中' | '错误' {
  if (sending) return '运行中'
  return latestMessageKind === 'error' ? '错误' : '就绪'
}

const AI_CHAT_FOCUSABLE_SELECTOR = [
  'a[href]',
  'area[href]',
  'button',
...
function isVisibleAiChatFocusTarget(element: HTMLElement, root: HTMLElement): boolean {
...
export function resolveAiChatTabWrapTarget(
  root: HTMLElement,
  activeElement: Element | null,
  shiftKey: boolean,
): HTMLElement | null {
  const focusTargets = Array.from(root.querySelectorAll<HTMLElement>(AI_CHAT_FOCUSABLE_SELECTOR))
    .filter((element) => isVisibleAiChatFocusTarget(element, root))
...
const AttachMenuTarget = React.forwardRef<HTMLButtonElement, AttachMenuTargetProps>(function AttachMenuTarget(
  { tooltip, tooltipDisabled = false, ...props },
  ref,
): JSX.Element {
  return (
    <Tooltip className="tc-ai-chat__tooltip" label={tooltip} disabled={tooltipDisabled} withArrow>
...
export default function AiChatDialog({ className }: { className?: string }): JSX.Element | null {
  ...
  const narrowViewport = useMediaQuery('(max-width: 56.25em)', false, { getInitialValueInEffect: false }) === true
  const [mode, setMode] = React.useState<AiChatMode>(initialLayoutPreference.mode)
  ...
  const previousModeRef = React.useRef<AiChatMode>(initialLayoutPreference.mode)
  const focusBeforeMaximizeRef = React.useRef<HTMLElement | null>(null)
  const focusRestoreModeRef = React.useRef<AiChatMode>(initialLayoutPreference.mode)
  const pendingBubbleFocusRestoreRef = React.useRef(false)
  ...
  const [attachMenuOpened, setAttachMenuOpened] = React.useState(false)
  const [taskMenuOpened, setTaskMenuOpened] = React.useState(false)
  ...
  const presentationMode = resolveAiChatPresentationMode(mode, narrowViewport)
  const isCompact = presentationMode === 'compact'
  const isMaximized = presentationMode === 'maximized'
  const showDockedBubble = dockRight && isCompact && (narrowViewport || bubbleVisualState === 'bubble')
  const canShowHistory = presentationMode === 'expanded' || presentationMode === 'maximized'
  ...
  const rememberFocusBeforeMaximize = React.useCallback(() => {
    if (typeof document === 'undefined') return
    focusBeforeMaximizeRef.current = document.activeElement instanceof HTMLElement
      ? document.activeElement
      : null
  }, [])

  const expandChat = React.useCallback(() => {
    if (narrowViewport && mode !== 'maximized') {
      rememberFocusBeforeMaximize()
    }
    setMode((m) => {
      if (narrowViewport) {
        if (m === 'maximized') return m
  ...
  }, [mode, narrowViewport, rememberFocusBeforeMaximize])
  ...
  const toggleMaximized = React.useCallback(() => {
    if (mode === 'maximized') {
      setMode(modeBeforeMaximizeRef.current)
      return
    }
    rememberFocusBeforeMaximize()
    modeBeforeMaximizeRef.current = mode === 'expanded' ? 'expanded' : 'compact'
    setMode('maximized')
  }, [mode, rememberFocusBeforeMaximize])
  ...
  React.useEffect(() => {
    if (typeof window === 'undefined') return
    if (presentationMode !== 'expanded' && presentationMode !== 'maximized') return
  ...
  }, [clearCreationSession, presentationMode])

  React.useEffect(() => {
    const previousMode = focusRestoreModeRef.current
    focusRestoreModeRef.current = mode
    if (presentationMode !== 'compact') {
      pendingBubbleFocusRestoreRef.current = false
    }
    if (previousMode !== 'maximized' || mode === 'maximized') return
    if (typeof window === 'undefined' || typeof document === 'undefined') return

    const rafId = window.requestAnimationFrame(() => {
      const previousFocusedElement = focusBeforeMaximizeRef.current
      focusBeforeMaximizeRef.current = null
      pendingBubbleFocusRestoreRef.current = false
      ...
      if (presentationMode === 'compact') {
        const bubbleButton = document.querySelector<HTMLElement>('.tc-ai-chat__bubble-button')
        if (bubbleButton) {
          bubbleButton.focus({ preventScroll: true })
        } else {
          pendingBubbleFocusRestoreRef.current = true
        }
        return
      }

      document.querySelector<HTMLElement>('.tc-ai-chat__command[aria-label="聚焦"]')?.focus({ preventScroll: true })
    })
  ...
  React.useEffect(() => {
    if (!showDockedBubble || !pendingBubbleFocusRestoreRef.current) return
    if (typeof window === 'undefined' || typeof document === 'undefined') return

    const rafId = window.requestAnimationFrame(() => {
      if (!pendingBubbleFocusRestoreRef.current) return
      const bubbleButton = document.querySelector<HTMLElement>('.tc-ai-chat__bubble-button')
  ...
  const visualStatus = resolveAiChatVisualStatus(sending, messages[messages.length - 1]?.kind)
  ...
  const shouldYieldKeyboardEventToNestedOverlay = React.useCallback((
    target: EventTarget | null,
    currentTarget: HTMLDivElement,
  ): boolean => {
    if (replicatePickerOpened || tutorialOpened) return true
    if (!(target instanceof Element)) return false
    const nestedDialog = target.closest('[role="dialog"]')
    if (nestedDialog && nestedDialog !== currentTarget) return true
    return Boolean(target.closest('[role="menu"]'))
  }, [replicatePickerOpened, tutorialOpened])

  const onRootKeyDownCapture = React.useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
    if (shouldYieldKeyboardEventToNestedOverlay(e.target, e.currentTarget)) return
    if (e.key === 'Tab' && isMaximized) {
      const wrapTarget = resolveAiChatTabWrapTarget(
        e.currentTarget,
  ...
  }, [isMaximized, mode, shouldYieldKeyboardEventToNestedOverlay, toggleMaximized])

  const onRootKeyDown = React.useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
    if (shouldYieldKeyboardEventToNestedOverlay(e.target, e.currentTarget)) return
    if (mode !== 'maximized') return
    e.stopPropagation()
  }, [mode, shouldYieldKeyboardEventToNestedOverlay])

  const rootClassName = [
    'tc-ai-chat',
    `tc-ai-chat--${presentationMode}`,
  ...
  const auraClassName = [
    'tc-ai-chat__aura',
    presentationMode === 'compact' ? 'tc-ai-chat__aura--compact' : '',
    presentationMode === 'maximized' ? 'tc-ai-chat__aura--maximized' : '',
  ].filter(Boolean).join(' ')
  const composerShellClassName = [
    'tc-ai-chat__composer-shell',
    referenceImages.length > 0 ? 'tc-ai-chat__composer-shell--with-refs' : '',
  ].filter(Boolean).join(' ')
  const visualStatusRow = (
    <span
      className="tc-ai-chat__visual-status"
      data-status={visualStatus}
      role="status"
      aria-live="polite"
      aria-atomic="true"
    >
      <span className="tc-ai-chat__visual-status-dot" aria-hidden="true" />
      <span className="tc-ai-chat__visual-status-label">{visualStatus}</span>
    </span>
  )

  const attachMenu = (
    <Menu
      className="tc-ai-chat__attach-menu"
      position="top-start"
    ...
    >
      <Menu.Target>
        <AttachMenuTarget
          tooltip={$('添加参考图（从画布选择或上传）')}
          tooltipDisabled={attachMenuOpened}
        />
      </Menu.Target>
  ...
  const taskEntryMenuButton = (
    <Menu
      ...
      shadow="md"
      zIndex={UI_LAYERS.floatingPopover}
      onChange={setTaskMenuOpened}
    >
      <Menu.Target>
        <Tooltip
          className="tc-ai-chat__tooltip"
          label={$('选择快捷任务或能力')}
          disabled={taskMenuOpened}
          withArrow
        >
          <Button
  ...
  return (
    <>
      ...
      <FocusTrap active={isMaximized && !replicatePickerOpened && !tutorialOpened}>
        <div
          className={rootClassName}
          data-chat-mode={presentationMode}
          data-ux-floating
          role={isMaximized ? 'dialog' : undefined}
          aria-modal={isMaximized ? true : undefined}
          aria-label={isMaximized ? $('AI 对话聚焦模式') : undefined}
        ...
        {isCompact ? (
                ...
                <Tooltip className="tc-ai-chat__tooltip" label={sending ? $('AI 对话中…点击展开') : $('展开 AI 对话')} withArrow position="left">
                  ...
                  >
                    <span className="tc-ai-chat__bubble-core">
                      <IconMessageCircle className="tc-ai-chat__bubble-icon" size={20} />
                      <span className="tc-ai-chat__bubble-label">AI</span>
                    </span>
                  </button>
                </Tooltip>
                {visualStatusRow}
                ...
                >
                  ...
                  <Group className="tc-ai-chat__compact-right" gap={6} align="center" wrap="nowrap">
                    <Tooltip className="tc-ai-chat__tooltip" label={$('开启新对话')} withArrow>
                      <Button
                        className="tc-ai-chat__command"
                        variant="subtle"
                        size="xs"
                        aria-label="新对话"
                        leftSection={<IconMessagePlus className="tc-ai-chat__command-icon" size={16} />}
                    ...
                    <Tooltip className="tc-ai-chat__tooltip" label={$('教程')} withArrow>
                      <Button
                        ...
                        size="xs"
                        aria-label="教程"
                        leftSection={<IconBook2 className="tc-ai-chat__command-icon" size={16} />}
                    ...
                    <Tooltip className="tc-ai-chat__tooltip" label={$('展开')} withArrow>
                      <Button
                        ...
                        size="xs"
                        aria-label="展开"
                        leftSection={<IconChevronUp className="tc-ai-chat__command-icon" size={16} />}
                        onClick={expandChat}
                      >
                        {$('展开')}
                      </Button>
                    ...
                    <Tooltip className="tc-ai-chat__tooltip" label={$('聚焦')} withArrow>
                      <Button
                        ...
                        size="xs"
                        aria-label="聚焦"
                        leftSection={<IconArrowsMaximize className="tc-ai-chat__command-icon" size={16} />}
          ...
          <>
            <Group className="tc-ai-chat__header" justify="space-between" align="center" gap={10} wrap="nowrap">
              <div
                className="tc-ai-chat__title-button tc-ai-chat__title-button--static"
                role="heading"
                aria-level={2}
              >
              ...
              <Group className="tc-ai-chat__header-right" gap={6} align="center" wrap="nowrap">
                ...
                {!isMaximized && (
                  <Tooltip className="tc-ai-chat__tooltip" label={$('收起')} withArrow>
                    <Button
                      ...
                      size="xs"
                      aria-label="收起"
                      leftSection={<IconChevronDown className="tc-ai-chat__command-icon" size={16} />}
                ...
                <Tooltip className="tc-ai-chat__tooltip" label={isMaximized ? $('退出聚焦') : $('聚焦')} withArrow>
                  <Button
                    className="tc-ai-chat__command"
                    variant="subtle"
                    size="xs"
                    aria-label={isMaximized ? '退出聚焦' : '聚焦'}
                    leftSection={isMaximized
                      ? <IconArrowsMinimize className="tc-ai-chat__command-icon" size={16} />
                      : <IconArrowsMaximize className="tc-ai-chat__command-icon" size={16} />}
                    onClick={toggleMaximized}
                  >
                    {isMaximized ? $('退出聚焦') : $('聚焦')}
                  </Button>
                ...
                {isMaximized && (
                  <Tooltip className="tc-ai-chat__tooltip" label={$('关闭')} withArrow>
                    <Button
                      ...
                      size="xs"
                      aria-label="关闭"
                      leftSection={<IconX className="tc-ai-chat__command-icon" size={16} />}
                      onClick={collapseChat}
                    >
                      {$('关闭')}
                    </Button>
