...
function normalizeStoryboardDialogueLines(value: unknown): string[] {
...
function normalizeDialogueComparable(value: string): string {
  return String(value || '')
    .trim()
    .replace(/[\s\u3000"'\u201c\u201d\u2018\u2019\u300c\u300d\u300e\u300f\uff08\uff09()\uff0c,\u3002\uff01\uff1f!?:\uff1a\uff1b;\u3001\u2026\u00b7-]+/gu, '')
    .toLowerCase()
...
function dialogueCoreText(line: string): string {
...
function promptContainsDialogue(prompt: string, dialogueLine: string): boolean {
  const promptComparable = normalizeDialogueComparable(prompt)
  if (!promptComparable) return false
  const candidates = [dialogueLine, dialogueCoreText(dialogueLine)]
  return candidates.some((candidate) => {
    const comparable = normalizeDialogueComparable(candidate)
    return comparable.length > 0 && promptComparable.includes(comparable)
  })
}

function dialogueLineMatches(left: string, right: string): boolean {
  const leftComparable = normalizeDialogueComparable(left)
  const rightComparable = normalizeDialogueComparable(right)
...
function appendStoryboardDialogueToPrompt(prompt: string, dialogueLines: string[]): string {
  const basePrompt = String(prompt || '').trim()
  const missingDialogue = dialogueLines.filter((line) => !promptContainsDialogue(basePrompt, line))
  if (!missingDialogue.length) return basePrompt
  return [
    basePrompt,
    '\u5bf9\u767d\u8981\u6c42\uff1a',
    ...missingDialogue.map((line) => `- ${line}`),
  ].filter(Boolean).join('\n')
}
...
function buildSplitVideoNode(input: {
  ...
  const nextPrompt = appendStoryboardDialogueToPrompt((chunkPromptText || basePrompt).trim(), normalizeStoryboardDialogueLines(baseConfig.storyboardDialogue))
  return {
    ...input.node,
...
function hasChapterGroundingConfig(config: PlanNodeConfig): boolean {
  return Boolean(getStringField(config, 'sourceBookId') || getStringField(config, 'bookId')) &&
    (getChapterValueFromConfig(config) !== undefined || getNumberField(config, 'materialChapter') !== undefined)
}

function hasStructuredImagePromptSpec(config: PlanNodeConfig): boolean {
  return typeof config.imagePromptSpecV2 !== 'undefined' || typeof config.structuredPrompt !== 'undefined'
}

function normalizePlanDialogueConsistency(plan: ChatCanvasPlan): ChatCanvasPlan {
  ...
  for (const edge of plan.edges || []) {
    const upstream = upstreamById.get(edge.targetClientId)
    if (upstream) upstream.push(edge.sourceClientId)
  }

  const nextNodes = plan.nodes.map((node) => {
    const kind = normalizeNodeKind(node)
    if (!VIDEO_PLAN_KINDS.has(kind)) return node
    const config = getNodeConfig(node)
    const prompt = extractVideoExecutionPromptFromConfig(config)
    const ownDialogue = normalizeStoryboardDialogueLines(config.storyboardDialogue)
    const upstreamDialogue: string[] = []
    for (const upstreamId of upstreamById.get(node.clientId) || []) {
      const upstreamNode = nodeById.get(upstreamId)
      if (!upstreamNode) continue
      for (const line of normalizeStoryboardDialogueLines(getNodeConfig(upstreamNode).storyboardDialogue)) {
        if (!upstreamDialogue.some((existing) => dialogueLineMatches(existing, line))) upstreamDialogue.push(line)
      }
    }

    if (upstreamDialogue.length) {
      const promptKeepsUpstreamDialogue = upstreamDialogue.every((line) => promptContainsDialogue(prompt, line))
      const ownKeepsUpstreamDialogue = upstreamDialogue.every((line) => ownDialogue.some((ownLine) => dialogueLineMatches(ownLine, line)))
      if (!promptKeepsUpstreamDialogue && !ownKeepsUpstreamDialogue) {
        throw new Error('\u753b\u5e03\u8ba1\u5212\u6821\u9a8c\u5931\u8d25\uff1a\u89c6\u9891\u8282\u70b9\u6ca1\u6709\u5728 config.prompt \u4e2d\u4fdd\u7559\u4e0a\u6e38\u5173\u952e\u53f0\u8bcd')
      }
    }

    if (!ownDialogue.length) return node
    const nextPrompt = appendStoryboardDialogueToPrompt(prompt, ownDialogue)
...
function validateCanvasPlan(plan: ChatCanvasPlan): ChatCanvasPlan {
  const normalizedPlan = normalizePlanDialogueConsistency(normalizePlanNovelTraceability(normalizeCanvasPlanVideoChunks(plan)))
  for (const node of normalizedPlan.nodes) {
    ...
    const config = getNodeConfig(node)
    if (kind === 'image' && hasChapterGroundingConfig(config) && !hasStructuredImagePromptSpec(config)) {
      throw new Error(`画布计划校验失败：节点「${node.label}」(${kind}) 缺少 config.imagePromptSpecV2；章节关联图像节点必须提供结构化图像提示词。`)
    }
    if (!prompt) {
