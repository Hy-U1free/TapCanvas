...
import { calculateHeaderSafeTop } from '../ui/utils/panelPosition'
import { resolveViewportFloatingPosition } from '../ui/utils/floatingPosition'
  ...
  return (
    <CanvasRenderContext.Provider value={canvasRenderContextValue}>
      ...
      {!viewOnly && referencePickerTargetId && (
        ...
        >
          <Group className="tc-canvas__reference-picker-bar-group" gap={8} wrap="wrap">
      ...
      {menu?.show && typeof document !== 'undefined' && (() => {
        const headerRect = document.querySelector('.app-header-overlay')?.getBoundingClientRect()
        const safeTop = Math.max(8, (headerRect?.bottom || 0) + 8)
        const availableHeight = Math.max(48, window.innerHeight - safeTop - 8)
        const menuWidth = Math.min(280, Math.max(220, window.innerWidth - 16))
        const maximumTop = availableHeight < 420
          ? safeTop
          : Math.max(safeTop, window.innerHeight - 420)
        const menuTop = Math.max(safeTop, Math.min(menu.y, maximumTop))
        const menuLeft = Math.max(8, Math.min(menu.x, window.innerWidth - menuWidth - 8))

        return createPortal(<PanelCard
          ...
          style={{
            ...
            left: menuLeft,
            top: menuTop,
            zIndex: UI_LAYERS.fullscreenWorkspace,
            width: menuWidth,
            minWidth: Math.min(220, window.innerWidth - 16),
        ...
        </PanelCard>, document.body)
      })()}
      {insertMenuContent && typeof document !== 'undefined' && (() => {
        const headerRect = document.querySelector('.app-header-overlay')?.getBoundingClientRect()
        const safeTop = Math.max(8, (headerRect?.bottom || 0) + 8)
        const availableHeight = Math.max(48, window.innerHeight - safeTop - 8)
        const menuWidth = Math.min(280, Math.max(220, window.innerWidth - 16))
        const maximumTop = availableHeight < 320
          ? safeTop
          : Math.max(safeTop, window.innerHeight - 320)
        const menuTop = Math.max(safeTop, Math.min(insertMenu.y + 10, maximumTop))
        const menuLeft = Math.max(8, Math.min(insertMenu.x + 10, window.innerWidth - menuWidth - 8))
      ...
      {connectingType && typeof document !== 'undefined' && (() => {
        const width = Math.min(280, Math.max(1, window.innerWidth - 16))
        const position = resolveViewportFloatingPosition({
          anchorLeft: mouse.x + 12,
          anchorTop: mouse.y,
        ...
        return createPortal(
          <div
            className="tc-canvas__connecting-tooltip"
            style={{
              position: 'fixed',
              left: position.left,
              top: position.top,
...
const CanvasSelectionActionBar = React.memo(function CanvasSelectionActionBar({
  ...
  const recomputePosition = React.useCallback(() => {
    ...
    const floatingWidth = Math.min(element.offsetWidth, Math.max(1, container.clientWidth - 16))
    const floatingHeight = Math.min(element.offsetHeight, Math.max(1, container.clientHeight - 16))
    const containerRect = container.getBoundingClientRect()
    const safeTop = Math.max(8, calculateHeaderSafeTop() - containerRect.top)
    const nextPosition = resolveViewportFloatingPosition({
      anchorLeft: anchor.centerX * zoom + tx - floatingWidth / 2,
      anchorTop: anchor.topY * zoom + ty,
      anchorBottom: anchor.bottomY * zoom + ty,
      floatingWidth,
      floatingHeight,
  ...
  }, [anchor.bottomY, anchor.centerX, anchor.topY, tx, ty, zoom])

  React.useLayoutEffect(() => {
    recomputePosition()
  }, [children, recomputePosition])

  React.useEffect(() => {
    const element = barRef.current
    const container = element?.offsetParent
    const observer = typeof ResizeObserver === 'function'
      ? new ResizeObserver(recomputePosition)
      : null
