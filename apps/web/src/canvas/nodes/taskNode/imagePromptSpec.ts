...
type ResolveStructuredPromptOptions = {
  allowLegacyObjectShape: boolean
  errorLabel?: string
}

function resolveStructuredPrompt(
  record: UnknownRecord,
  options: ResolveStructuredPromptOptions = { allowLegacyObjectShape: true },
): {
  ...
  for (const candidate of candidates) {
    ...
    sawCandidate = true
    if (!firstKey) firstKey = candidate.key
  ...
  if (options.allowLegacyObjectShape) {
  ...
  if (sawCandidate) {
    const label = options.errorLabel || firstKey || 'structuredPrompt'
    throw new Error(`${label} 非法：${firstError || '缺少有效结构化内容'}`)
  }
...
function resolveImagePromptExecutionInternal(
  data: unknown,
  options: ResolveStructuredPromptOptions,
): ResolvedImagePromptExecution {
  const record = asRecord(data)
  if (!record) {
    return {
      prompt: '',
  ...
  const resolvedStructuredPrompt = resolveStructuredPrompt(record, options)

  if (explicitMode === 'structured') {
    if (!resolvedStructuredPrompt?.structuredPrompt) {
      throw new Error(`${options.errorLabel || 'structuredPrompt'} 非法：structuredPrompt 模式缺少有效结构化内容`)
    }
    return {
...
export function resolveImagePromptExecution(data: unknown): ResolvedImagePromptExecution {
  return resolveImagePromptExecutionInternal(data, {
    allowLegacyObjectShape: true,
    errorLabel: 'structuredPrompt',
  })
}
...
function normalizeResolvedImagePromptConfig(config: UnknownRecord, resolved: ResolvedImagePromptExecution): UnknownRecord {
  const {
    imagePromptSpecV2: _imagePromptSpecV2,
    imagePromptSpec: _imagePromptSpec,
    promptSpec: _promptSpec,
    structuredPrompt: _structuredPrompt,
    ...restConfig
  } = config
  ...
  return {
    ...
    ...(resolved.structuredPrompt
      ? {
          imagePromptSpecV2: resolved.structuredPrompt,
          structuredPrompt: resolved.structuredPrompt,
        }
      : null),
...
export function normalizeImagePromptExecutionConfig(config: UnknownRecord): UnknownRecord {
  const resolved = resolveImagePromptExecutionInternal(config, {
    allowLegacyObjectShape: false,
    errorLabel: 'imagePromptSpecV2',
  })
  return normalizeResolvedImagePromptConfig(config, resolved)
}
