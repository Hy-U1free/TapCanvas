...
export function PromptSection({
  ...
  const [suggestionMenuPosition, setSuggestionMenuPosition] = React.useState<MentionMenuPosition | null>(null)
  ...
  const updateMentionMenuPosition = React.useCallback(() => {
    ...
    const preferredWidth = Math.min(
      ...
      Math.max(220, textareaRect.width - 16),
    )
    const maxHeight = Math.min(220, Math.max(1, window.innerHeight - 16))
    const position = resolveViewportFloatingPosition({
      anchorLeft: caretMetrics.left,
      anchorTop: caretMetrics.top,
      anchorBottom: caretMetrics.top + caretMetrics.lineHeight,
  ...
  const updateSuggestionMenuPosition = React.useCallback(() => {
    const textarea = textareaRef.current
    if (!textarea) {
    ...
    const width = Math.min(
      340,
      Math.max(1, window.innerWidth - 16),
      Math.max(220, textareaRect.width),
    )
    const maxHeight = Math.min(180, Math.max(1, window.innerHeight - 16))
    const position = resolveViewportFloatingPosition({
      anchorLeft: textareaRect.right - width,
      anchorTop: textareaRect.top,
      anchorBottom: textareaRect.bottom,
      floatingWidth: width,
      floatingHeight: maxHeight,
      viewportWidth: window.innerWidth,
      viewportHeight: window.innerHeight,
  ...
  React.useEffect(() => {
    if (!allowPromptEditing || mentionOpen || promptSuggestions.length === 0) {
      setSuggestionMenuPosition(null)
      return
    }
    updateSuggestionMenuPosition()
    const handleViewportChange = () => updateSuggestionMenuPosition()
    window.addEventListener('resize', handleViewportChange)
    window.addEventListener('scroll', handleViewportChange, true)
    return () => {
      window.removeEventListener('resize', handleViewportChange)
  ...
  }, [allowPromptEditing, mentionOpen, promptSuggestions.length, updateSuggestionMenuPosition])
  ...
  return (
    <div className={rootClassName}>
      ...
      <div className="task-node-prompt__input-wrap" ref={inputWrapRef} style={{ position: 'relative' }}>
        ...
        {allowPromptEditing && !mentionOpen && promptSuggestions.length > 0 && suggestionMenuPosition && (
          <Portal>
            <div
        ...
        {allowPromptEditing && mentionOpen && mentionMenuPosition && (
          <Portal>
            <div
