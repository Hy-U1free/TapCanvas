...
export function TopToolbar({
  ...
  const toolbarPosition = useHeaderAwareToolbarPosition()
  if (!isVisible || !hasContent) return null
  const allActions = [...extraActions, ...visibleDefs]
  return (
    <NodeToolbar className="top-toolbar" position={toolbarPosition} align="center">
...
function useHeaderAwareToolbarPosition(): Position {
  const nodeId = useNodeId()
  const nodeTop = useStore(React.useCallback((state) => {
    if (!nodeId) return null
    const node = state.nodeLookup.get(nodeId)
    if (!node) return null
    return node.internals.positionAbsolute.y * state.transform[2] + state.transform[1]
  }, [nodeId]))
  if (typeof document === 'undefined' || typeof nodeTop !== 'number') return Position.Top
  const estimatedToolbarHeight = 58
  return nodeTop - estimatedToolbarHeight <= calculateHeaderSafeTop()
    ? Position.Bottom
    : Position.Top
}
