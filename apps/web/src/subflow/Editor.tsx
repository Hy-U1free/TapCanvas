...
export default function SubflowEditor({ nodeId, onClose }: Props) {
  ...
  if (!open) return null
  return (
    <div className="tc-subflow-editor" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', zIndex: UI_LAYERS.fullscreenWorkspace, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="tc-subflow-editor__panel" ref={rootRef} style={{ width: '88%', height: '88%', background: 'var(--mantine-color-default)', color: 'inherit', borderRadius: 12, overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,.35)', border: '1px solid rgba(127,127,127,.25)' }}>
        <div className="tc-subflow-editor__header" style={{ padding: 10, borderBottom: '1px solid rgba(127,127,127,.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
