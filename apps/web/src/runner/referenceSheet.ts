...
function loadDirectReferenceImageSource(url: string): Promise<LoadedReferenceImageSource> {
  return new Promise((resolve, reject) => {
    if (typeof Image === 'undefined') {
      reject(new Error('Image element is not available'))
      return
    }
    ...
    image.onerror = () => reject(new Error('direct image element load failed'))
    image.src = url
  })
}
...
export async function loadReferenceSheetImageSource(url: string): Promise<LoadedReferenceImageSource> {
  try {
    return await loadDirectReferenceImageSource(url)
  } catch {
    // Fall through to the shared resource loader so production still benefits from
    // fetch/blob decoding and runtime cache handling when direct element loading fails.
  }
