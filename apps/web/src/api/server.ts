...
function throwAuthRequestError(response: Response, body: AuthErrorBody | null, fallback: string): never {
  const message = (body && (body.error || body.message)) || fallback
  const error: AuthRequestError = new Error(message)
...
export type EmailRegistrationCodeResponse = {
  sent: boolean
  expiresInSeconds?: number
  retryAfterSeconds?: number
  devCode?: string
...
export async function requestEmailRegistrationCode(email: string): Promise<EmailRegistrationCodeResponse> {
  const r = await apiFetch(`${API_BASE}/auth/email/register/request`, {
    method: 'POST',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!r.ok) {
    throwAuthRequestError(r, body, `email registration code request failed: ${r.status}`)
  }
  return {
    ...
    delivery: body?.delivery === 'debug' ? 'debug' : body?.delivery === 'email' ? 'email' : undefined,
  }
}

export async function registerWithEmail(email: string, code: string, password: string): Promise<AuthResponseDto> {
  const r = await apiFetch(`${API_BASE}/auth/email/register/verify`, {
    method: 'POST',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
  ...
  const body = await parseAuthErrorBody(r)
  if (!r.ok) {
    throwAuthRequestError(r, body, `email registration failed: ${r.status}`)
  }
  return body as AuthResponseDto
}

export async function loginWithEmailPassword(email: string, password: string): Promise<AuthResponseDto> {
  const r = await apiFetch(`${API_BASE}/auth/email/password-login`, {
    method: 'POST',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
  ...
  const body = await parseAuthErrorBody(r)
  if (!r.ok) {
    throwAuthRequestError(r, body, `email password login failed: ${r.status}`)
  }
  return body as AuthResponseDto
}
  ...
  if (!r.ok) {
    try {
      ...
      if (e.code === 'team_required') {
        e.message = '个人账号也可使用，但需要有积分；新用户通过邮箱注册赠送 100 积分。'
      }
      throw e
    }
  }
  ...
  if (!r.ok) {
    ...
    if (body && typeof body === 'object') {
      ...
      if (err.code === 'team_required') {
        err.message =
          '个人账号也可使用，但需要有积分；新用户通过邮箱注册赠送 100 积分。'
      }
    }
    throw err
  }
