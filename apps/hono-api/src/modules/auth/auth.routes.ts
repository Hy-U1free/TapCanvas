...
authRouter.post("/email/register/request", async (c) => {
	const body = (await c.req.json().catch(() => ({}))) ?? {};
	const parsed = EmailRegisterRequestSchema.safeParse(body);
	if (!parsed.success) {
		return c.json(
	...
	const result = await requestEmailRegistrationCode(c, parsed.data.email);
	if (result instanceof Response) return result;
	return c.json({ success: true, ...result });
});

authRouter.post("/email/register/verify", async (c) => {
	const body = (await c.req.json().catch(() => ({}))) ?? {};
	const parsed = EmailRegisterVerifyRequestSchema.safeParse(body);
	if (!parsed.success) {
		return c.json(
			{ success: false, error: "请求参数不合法", issues: parsed.error.issues },
			400,
...
authRouter.post("/email/password-login", async (c) => {
	const body = (await c.req.json().catch(() => ({}))) ?? {};
	const parsed = EmailPasswordLoginRequestSchema.safeParse(body);
	if (!parsed.success) {
		return c.json(
			{ success: false, error: "请求参数不合法", issues: parsed.error.issues },
			400,
