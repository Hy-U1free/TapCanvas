...
const EMAIL_REGISTRATION_PURPOSE = "register";
const EMAIL_REGISTRATION_OTP_TTL_SECONDS = 10 * 60;
const EMAIL_REGISTRATION_OTP_RETRY_SECONDS = 60;
const EMAIL_REGISTRATION_OTP_DAILY_LIMIT = 10;
...
async function issueAuthPayload(
	...
	persistedAuthState?: PersistedAuthState,
): Promise<{ token: string; user: UserPayload }> {
	const config = getConfig(c.env);
	const userRow = persistedAuthState
		? {
				role: persistedAuthState.role,
				password_hash: persistedAuthState.passwordHash,
			}
		: await getPrismaClient().users.findUnique({
...
async function sendEmailOtpIfConfigured(options: {
	...
	const { c, to, code, purpose = "login" } = options;
	const config = getConfig(c.env);
	...
	try {
		resp = await fetch("https://api.resend.com/emails", {
			...
			body: JSON.stringify({
				...
				text:
					purpose === "register"
						? `你的 TapCanvas 注册验证码：${code}\n\n10 分钟内有效。`
						: `你的 TapCanvas 登录验证码：${code}\n\n10 分钟内有效。`,
			}),
		});
	} catch (error) {
...
function resolveRuntimeNodeEnv(c: AppContext): string {
	const contextValue = String(c.env.NODE_ENV || "").trim().toLowerCase();
	if (contextValue) return contextValue;
	return String(globalThis.process?.env.NODE_ENV || "").trim().toLowerCase();
}

function isEmailRegistrationDebugEnabled(
	c: AppContext,
	config: ReturnType<typeof getConfig>,
): boolean {
	const runtime = resolveRuntimeNodeEnv(c);
	return ["development", "test", "local"].includes(runtime) && config.emailLoginDebug;
}
...
function isRetryableAuthTransactionError(error: unknown): boolean {
	if (error && typeof error === "object" && "code" in error) {
		const code = String((error as { code?: unknown }).code || "");
		if (code === "P2034") return true;
	}
	const message = error instanceof Error ? error.message : String(error || "");
	return /serialization|write conflict|deadlock/i.test(message);
}

async function runSerializableAuthTransaction<T>(
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
		try {
			...
			if (attempt >= maxAttempts || !isRetryableAuthTransactionError(error)) {
				throw error;
			}
		}
	}
...
function emailRegistrationConfigurationError(
	c: AppContext,
	missing: {
		AUTH_OTP_PEPPER: boolean;
...
export async function requestEmailRegistrationCode(
	...
	const isDebug = isEmailRegistrationDebugEnabled(c, config);
	const pepper = String(config.authOtpPepper || "").trim();
	const missing = {
		AUTH_OTP_PEPPER: !pepper,
		RESEND_API_KEY: !config.resendApiKey,
		RESEND_FROM: !config.resendFrom,
	};
	if (missing.AUTH_OTP_PEPPER || (!isDebug && (missing.RESEND_API_KEY || missing.RESEND_FROM))) {
		return emailRegistrationConfigurationError(c, missing);
	}
	...
	const codeHash = await hmacSha256Hex(pepper, `${salt}:${code}`);
	const expiresAtIso = new Date(
		now + EMAIL_REGISTRATION_OTP_TTL_SECONDS * 1000,
	).toISOString();
	type EmailCodeIssuanceResult =
		| { ok: true }
		| { ok: false; response: Response };
	const issuance = await runSerializableAuthTransaction(
		prisma,
		async (transaction): Promise<EmailCodeIssuanceResult> => {
			const latestRequest = await transaction.email_login_codes.findFirst({
				where: { email, purpose: EMAIL_REGISTRATION_PURPOSE },
				orderBy: { created_at: "desc" },
				select: { created_at: true },
			});
			if (latestRequest) {
				const latestCreatedAt = Date.parse(latestRequest.created_at);
				if (Number.isFinite(latestCreatedAt)) {
					const retryAfterSeconds = Math.ceil(
						(latestCreatedAt + EMAIL_REGISTRATION_OTP_RETRY_SECONDS * 1000 - now) /
							1000,
					);
					if (retryAfterSeconds > 0) {
						return {
							...
							response: c.json(
								{
									success: false,
									error: `验证码发送过于频繁，请 ${retryAfterSeconds} 秒后再试`,
			...
			const requestCount = await transaction.email_login_codes.count({
				where: {
					email,
					purpose: EMAIL_REGISTRATION_PURPOSE,
					created_at: { gte: getChinaDayStartIso(now) },
				},
			});
			if (requestCount >= EMAIL_REGISTRATION_OTP_DAILY_LIMIT) {
				return {
					ok: false,
					response: c.json(
						{
							success: false,
							error: `该邮箱今日验证码请求次数已达上限（${EMAIL_REGISTRATION_OTP_DAILY_LIMIT} 次），请明天再试`,
							code: "email_registration_daily_limit_exceeded",
							details: {
								limitPerDay: EMAIL_REGISTRATION_OTP_DAILY_LIMIT,
			...
			await transaction.email_login_codes.updateMany({
				where: {
					email,
					purpose: EMAIL_REGISTRATION_PURPOSE,
					used_at: null,
			...
			await transaction.email_login_codes.create({
				data: {
					id,
					email,
					purpose: EMAIL_REGISTRATION_PURPOSE,
	...
	if (isDebug) {
		return {
			sent: true,
			expiresInSeconds: EMAIL_REGISTRATION_OTP_TTL_SECONDS,
			retryAfterSeconds: EMAIL_REGISTRATION_OTP_RETRY_SECONDS,
			devCode: code,
			delivery: "debug" as const,
		};
	}

	const delivery = await sendEmailOtpIfConfigured({
		c,
		to: email,
		code,
		purpose: "register",
	...
	if (!delivery.sent) {
		...
		await prisma.email_login_codes.updateMany({
			where: { id, used_at: null },
			data: { used_at: failedAt, updated_at: failedAt },
		});
		return c.json(
			{
				success: false,
...
async function findEmailAccounts(
	email: string,
	client: Prisma.TransactionClient =
		getPrismaClient() as unknown as Prisma.TransactionClient,
	...
	const rows = await client.users.findMany({
		...
		select: EMAIL_ACCOUNT_SELECT,
		take: 2,
	});
	return rows as EmailAccountRow[];
}

function emailAccountConflict(c: AppContext): Response {
	return c.json(
		{
			success: false,
			error: "该邮箱关联了多个账号，请联系管理员处理",
...
function rejectUnavailableEmailAccount(
	c: AppContext,
	account: EmailAccountRow,
): Response | null {
	if (account.deleted_at) {
		return c.json(
			{ success: false, error: "账号已被删除", code: "user_deleted" },
			403,
		);
	}
	if (Number(account.disabled ?? 0) !== 0) {
		return c.json(
			{ success: false, error: "账号已被禁用", code: "user_disabled" },
			403,
		);
	}
	return null;
...
function invalidEmailPasswordCredentials(c: AppContext): Response {
	return c.json(
...
async function runDummyEmailPasswordVerification(password: string): Promise<void> {
	await verifyPasswordRecord({
		password,
		hash: "0".repeat(64),
		salt: "tapcanvas-email-login-dummy-salt-v1",
	});
}

async function reconcileEmailSignupBonus(
	c: AppContext,
	userId: string,
): Promise<void> {
	try {
		await grantSignupBonusToPersonalTeam(c, userId);
	} catch (error) {
		console.error("[auth/email] signup bonus reconciliation failed", {
			userId,
			error,
		});
	}
...
function invalidEmailRegistrationCode(c: AppContext): Response {
	return c.json(
		{
			success: false,
			error: "验证码不正确或已过期",
...
export async function verifyEmailRegistration(
	...
	const pepper = String(config.authOtpPepper || "").trim();
	if (!pepper) {
		return emailRegistrationConfigurationError(c, {
	...
	if (!row) return invalidEmailRegistrationCode(c);
	if (Number(row.attempt_count || 0) >= 5) {
		await prisma.email_login_codes.updateMany({
			where: { id: row.id, used_at: null },
			data: { used_at: nowIso, updated_at: nowIso },
		});
		return invalidEmailRegistrationCode(c);
	}

	const actualHash = await hmacSha256Hex(
		pepper,
		`${String(row.code_salt || "")}:${String(code || "").trim()}`,
	);
	const expectedHash = String(row.code_hash || "");
	if (!expectedHash || !constantTimeHexEqual(expectedHash, actualHash)) {
		const nextAttemptCount = Number(row.attempt_count || 0) + 1;
		const incremented = await prisma.email_login_codes.updateMany({
			where: {
				id: row.id,
				used_at: null,
				attempt_count: { lt: 5 },
			...
			data: {
				...
				updated_at: nowIso,
				...(nextAttemptCount >= 5 ? { used_at: nowIso } : {}),
			},
		});
	...
	type RegistrationTransactionResult =
		| { ok: true; account: EmailAccountRow }
		| { ok: false; response: Response };
	const transactionResult = await runSerializableAuthTransaction(
		prisma,
		async (transaction): Promise<RegistrationTransactionResult> => {
			const accounts = await findEmailAccounts(email, transaction);
			if (accounts.length > 1) {
				return { ok: false, response: emailAccountConflict(c) };
			}
			const existingAccount = accounts[0] ?? null;
			if (existingAccount) {
				const unavailable = rejectUnavailableEmailAccount(c, existingAccount);
			...
			if (consumed.count !== 1) {
				return { ok: false, response: invalidEmailRegistrationCode(c) };
			}
	...
	if (!transactionResult.ok) return transactionResult.response;
	const account = transactionResult.account;

	await reconcileEmailSignupBonus(c, account.id);
	return issueAuthPayload(c, {
		userId: account.id,
		login: account.login,
		...
		guest: Number(account.guest ?? 0) !== 0,
	}, {
		role: account.role,
...
export async function loginWithEmailPassword(
	...
	const email = normalizeAuthEmail(rawEmail);
	const accounts = await findEmailAccounts(email);
	if (accounts.length > 1) return emailAccountConflict(c);
	const account = accounts[0] ?? null;
	if (!account) {
		await runDummyEmailPasswordVerification(password);
		return invalidEmailPasswordCredentials(c);
	}

	const passwordHash = hasPasswordConfigured(account.password_hash)
		? String(account.password_hash)
		: "";
	const passwordSalt = String(account.password_salt || "");
	...
	const unavailable = rejectUnavailableEmailAccount(c, account);
	if (unavailable) return unavailable;

	const nowIso = new Date().toISOString();
	type EmailPasswordLoginTransactionResult =
		| { ok: true; account: EmailAccountRow }
		| { ok: false; response: Response };
	const transactionResult = await runSerializableAuthTransaction(
		getPrismaClient(),
		async (transaction): Promise<EmailPasswordLoginTransactionResult> => {
			...
			const current = (await transaction.users.findUnique({
				where: { id: account.id },
				select: EMAIL_ACCOUNT_SELECT,
			})) as EmailAccountRow | null;

			if (updated.count !== 1 || !current) {
				if (current) {
					const currentUnavailable = rejectUnavailableEmailAccount(c, current);
					if (currentUnavailable) {
						return { ok: false, response: currentUnavailable };
					}
				}
				return { ok: false, response: invalidEmailPasswordCredentials(c) };
			}
			if (
				current.password_hash !== passwordHash ||
				current.password_salt !== passwordSalt
			) {
				return { ok: false, response: invalidEmailPasswordCredentials(c) };
			}
	...
	if (!transactionResult.ok) return transactionResult.response;
	const currentAccount = transactionResult.account;

	await reconcileEmailSignupBonus(c, currentAccount.id);
	return issueAuthPayload(c, {
		userId: currentAccount.id,
		login: currentAccount.login,
		name: currentAccount.name || currentAccount.login,
		avatarUrl: currentAccount.avatar_url ?? null,
		email,
		phone: currentAccount.phone ?? null,
		guest: Number(currentAccount.guest ?? 0) !== 0,
	}, {
		role: currentAccount.role,
		passwordHash: currentAccount.password_hash,
	});
}
