...
export async function grantTeamSignupBonusOnce(
	db: PrismaClient,
	input: {
		teamId: string;
		amount: number;
	...
	if (!teamId || !actorUserId || amount <= 0) return false;
	const idempotencyKey = `signup_bonus:${actorUserId}`;
	...
	return db.$transaction(async (transaction) => {
       ...
       SELECT $1, $2, 'topup', $3, $4, 'signup_bonus', $5, 'signup_bonus', $6
       WHERE NOT EXISTS (
         SELECT 1
         FROM team_credit_ledger
         WHERE team_id = $2
       ...
       WHERE id = $3`,
		...
		if (Number(updated) !== 1) {
			throw new Error("signup bonus team not found");
		}
		return true;
	});
}
