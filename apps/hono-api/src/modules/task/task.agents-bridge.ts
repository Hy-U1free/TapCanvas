...
export async function runAgentsBridgeChatTask(
	...
	const selectedNodeKindForAgents = sanitizeStoryboardEditorKindForAgents(
		effectiveChatContext.selectedNodeKind,
	);
	const selectedReferenceForAgents = sanitizeSelectedReferenceForAgents(
		effectiveChatContext.selectedReference,
	);
	const requestFactDiagnosticContext: Record<string, unknown> = {
		...(effectiveChatContext.currentProjectName
			? { currentProjectName: effectiveChatContext.currentProjectName }
			: {}),
		...(effectiveChatContext.selectedNodeLabel
			? { selectedNodeLabel: effectiveChatContext.selectedNodeLabel }
			: {}),
		...(selectedNodeKindForAgents
			? { selectedNodeKind: selectedNodeKindForAgents }
			: {}),
		...(effectiveChatContext.selectedNodeTextPreview
			? { selectedNodeTextPreview: effectiveChatContext.selectedNodeTextPreview }
			: {}),
		...(selectedReferenceForAgents
			? { selectedReference: selectedReferenceForAgents }
			: {}),
		...(planOnly ? { planOnly: true } : {}),
		...(forceAssetGeneration ? { forceAssetGeneration: true } : {}),
	};
		...
		const init: any = {
			...
			headers: {
				...
				body: JSON.stringify({
					...
					...(canvasProjectId || canvasNodeId || bookId || chapterId || chunkIndex !== null || groupSize !== null || shotStart !== null || shotEnd !== null || shotNo !== null || diagnosticsLabel || executionPlanningDirective || Object.keys(requestFactDiagnosticContext).length > 0
						? {
							diagnosticContext: {
